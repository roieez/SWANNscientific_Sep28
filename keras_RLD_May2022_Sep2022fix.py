...
def multip_LRs_dessications(type, dataset_lng, part, regularizer=0):
    ...
    for i in range(dessications):
        ...
        r2_after_nullify[i] = r2_score(dataset[:predict_until, -1], predict_by_wts)
...
# # total switch with three states
#
# inputs = tf.keras.Input(shape=(int(dataset_lng.shape[1] - 1)), name="dataset")
# inputs_norm = normalizer_shrt(inputs[:, :take_coeffs])  # if only part of coefficients are desired
...
#
# total_switch_model = tf.keras.Model(inputs, outputs, name="total_slim_model")
#
# total_switch_model.summary()

# def custom_max(x):
#     # return tf.sparse.to_dense(tf.argmax(x), tf.cast(tf.shape(x), dtype=tf.int64), tf.reduce_max(x)) / tf.reduce_max(x)
#     # y = tf.abs(x) / tf.reduce_max(tf.abs(x))
#     y = tf.nn.softmax(x)
#     return tf.floor(y / tf.reduce_max(y))
#
#
# def custom_binary(x):
#     y = np.zeros_like(x)
#     y[np.where(x==np.max(x))] = 1
#     return y

# def build_custom_activation(threshold):
#     """
#     builds an activation function that rectifies |x| < threshold
#     :param threshold: numpy float denoting the threshold from which to rectify
#     :return: activation function
#     """
#
#     def custom_activation(x):
#         return tf.keras.activations.relu(x, threshold=threshold) - tf.keras.activations.relu(-x, threshold=threshold)
#     return custom_activation

# def sparse_loss(y_true, y_pred, wts, alpha, beta):
#     squared_diff = tf.square(y_true - y_pred)
#     wts_binary = tf.greater(wts, 1e-3)
#     return alpha * tf.reduce_mean(squared_diff) + beta * tf.reduce_sum(wts_binary)

# def plot_weight_scatter(wts, weight):
#     if len(weight) == 7:
#         fig3, ax3 = plt.subplots(3, 3, figsize=(16, 16))
#         for i in range(len(wts) - 1):
#             if not len(wts) - 1:
#                 pass
#             else:
#                 for j in range(3):
#                     ax3[j, 0].plot(wts[i][:, j], wts[i+1][:, j], '.')
#                     ax3[j, 0].plot([min(wts[i][:, j]), max(wts[i][:, j])], [min(wts[i][:, j]), max(wts[i][:, j])])
#                     ax3[j, 1].plot(wts[np.mod(i, len(wts))][:, j], wts[np.mod(i+2, len(wts))][:, j], '.')
#                     ax3[j, 1].plot([min(wts[i][:, j]), max(wts[i][:, j])], [min(wts[i][:, j]), max(wts[i][:, j])])
#                     ax3[j, 2].plot(wts[np.mod(i+1, len(wts))][:, j], wts[np.mod(i+2, len(wts))][:, j], '.')
#                     ax3[j, 2].plot([min(wts[i][:, j]), max(wts[i][:, j])], [min(wts[i][:, j]), max(wts[i][:, j])])
#
#     if len(weight) == 6:
...
#                 pass
#             else:
#                 fig3.plot(wts[i], wts[i+1], '.')
#
#     pickle.dump(fig3, open('weights_against_weights' + str(take_coeffs) + '.pickle', 'wb'))

# # plot scatter of weights for each iteration relative to the previous iteration
# plot_weight_scatter(wts_ODE, weight)
# fig9, axes9 = plt.subplots(3, 1)
# axes9[0].plot(wts_ODE[0][:, 0])
# axes9[0].plot(wts_ODE[1][:, 0])
# axes9[1].plot(wts_ODE[0][:, 1])
# axes9[1].plot(wts_ODE[1][:, 1])
# axes9[2].plot(wts_ODE[0][:, 2])
# axes9[2].plot(wts_ODE[1][:, 2])